  ...
  requireCsrfToken();
    ...
    if ($action === 'add') {
        ...
        if ($employee_row) {
          ...
          $msg='<div style="color:var(--danger);padding:10px;background:rgba(239,68,68,0.1);border-radius:8px;margin-bottom:14px">🗑️ Employee deleted!</div>';
        }
      } elseif ($action === 'edit') {
        $id = sanitizeInt($_POST['id'] ?? 0);
        $name = sanitizeString($_POST['name'] ?? '', 200);
        ...
        $stmt = $conn->prepare("UPDATE employees SET name=?, role=?, phone=?, salary=?, nida=?, start_date=?, status=? WHERE id=?");
...
$employee_preview_rows = [];
$preview_result = $conn->query("SELECT name, role FROM employees ORDER BY name");
if ($preview_result) {
  while ($preview_row = $preview_result->fetch_assoc()) {
    $employee_preview_rows[] = $preview_row;
  }
}
?>
...
<div class="table-wrap"><table><thead><tr><th>#</th><th>Name</th><th>Phone</th><th>Salary</th><th>Start Date</th><th>Status</th><th>Actions</th></tr></thead>
<tbody><?php $i=1; while($e=$employees->fetch_assoc()):?>
<tr><td class="text-muted"><?=$i++?></td><td class="td-bold"><?=htmlspecialchars($e['name'])?></td><td><?=htmlspecialchars($e['phone'])?></td>
...
<td style="display:flex;gap:8px;align-items:center">
  <button type="button" class="btn btn-outline btn-sm" onclick='openEditEmployee(this)' data-id="<?=$e['id']?>" data-name="<?=htmlspecialchars($e['name'], ENT_QUOTES)?>" data-role="<?=htmlspecialchars($e['role'], ENT_QUOTES)?>" data-phone="<?=htmlspecialchars($e['phone'], ENT_QUOTES)?>" data-salary="<?=htmlspecialchars($e['salary'], ENT_QUOTES)?>" data-nida="<?=htmlspecialchars($e['nida'], ENT_QUOTES)?>" data-start="<?=htmlspecialchars($e['start_date'], ENT_QUOTES)?>" data-status="<?=htmlspecialchars($e['status'], ENT_QUOTES)?>">Edit</button>
  <form method="POST" onsubmit="return confirm('Delete <?=htmlspecialchars($e['name'], ENT_QUOTES)?>?')" style="margin:0">
...
<form method="POST"><input type="hidden" name="action" value="add"><input type="hidden" name="role" value="Cashier"><?= csrfInput() ?><div class="modal-body">
<div class="form-row"><div class="form-group"><label class="form-label">Full Name *</label><input name="name" class="form-control" required/></div><div class="form-group"></div></div>
...
<form method="POST"><input type="hidden" name="action" value="edit"><input type="hidden" name="id" id="edit-id"><input type="hidden" name="role" id="edit-role" value="Cashier"><?= csrfInput() ?><div class="modal-body">
<div class="form-row"><div class="form-group"><label class="form-label">Full Name *</label><input name="name" id="edit-name" class="form-control" required/></div><div class="form-group"></div></div>
...
<form method="POST" onsubmit="return confirm('Delete this employee permanently?')"><input type="hidden" name="action" value="delete"><?= csrfInput() ?><div class="modal-body">
