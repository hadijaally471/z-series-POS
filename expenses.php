...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  requireCsrfToken();
    ...
    if ($action === 'edit') {
        ...
        if ($id > 0 && $desc !== '') {
            $stmt = $conn->prepare("UPDATE expenses SET description=?, category=?, amount=?, expense_date=? WHERE id=?");
            $stmt->bind_param('ssdsi', $desc, $cat, $amount, $date, $id);
            $stmt->execute();
...
<div class="table-wrap"><table><thead><tr><th>#</th><th>Description</th><th>Category</th><th>Amount</th><th>Date</th><th>Actions</th></tr></thead>
...
<td style="display:flex;gap:6px;align-items:center">
  <button type="button" class="btn btn-outline btn-sm" onclick='openEditExpense(this)' data-id="<?=$e['id']?>" data-desc="<?=htmlspecialchars($e['description'], ENT_QUOTES)?>" data-cat="<?=htmlspecialchars($e['category'], ENT_QUOTES)?>" data-amount="<?=$e['amount']?>" data-date="<?=$e['expense_date']?>">Edit</button>
...
<?php endwhile; if($expenses->num_rows===0):?><tr><td colspan="6" style="text-align:center;padding:30px;color:var(--text3)">No expenses recorded</td></tr><?php endif;?></tbody></table></div></div>
