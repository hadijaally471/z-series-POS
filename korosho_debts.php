<?php
$page_title = 'Korosho Debts';
$content_class = 'content premium-content';
require_once 'includes/header.php';
requirePrivilege('korosho');
$msg = '';
if (!empty($_SESSION['korosho_debts_flash'])) {
  $msg = $_SESSION['korosho_debts_flash'];
  unset($_SESSION['korosho_debts_flash']);
}
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  requireCsrfToken();
    ...
    if ($amount <= 0) {
      ...
      try {
        $stmt = $conn->prepare("SELECT * FROM korosho_debts WHERE id = ? FOR UPDATE");
      ...
      if ($new_due !== '' && $status !== 'cleared') {
        $stmt_up = $conn->prepare("UPDATE korosho_debts SET amount_paid = ?, balance = ?, status = ?, due_date = ? WHERE id = ?");
        $stmt_up->bind_param('ddssi', $new_paid, $bal, $status, $new_due, $debt_id);
      } else {
        $stmt_up = $conn->prepare("UPDATE korosho_debts SET amount_paid = ?, balance = ?, status = ? WHERE id = ?");
        $stmt_up->bind_param('ddsi', $new_paid, $bal, $status, $debt_id);
      }
      $stmt_up->execute();
      $stmt = $conn->prepare("INSERT INTO korosho_debt_payments (debt_id,amount,payment_method) VALUES (?,?,?)");
      $stmt->bind_param('ids',$debt_id,$amount,$method); $stmt->execute();
      if ($debt['customer_id']) {
        $custId = (int)$debt['customer_id'];
        $dec = min($amount,$debt['balance']);
        $stmt_c = $conn->prepare("UPDATE korosho_customers SET outstanding_debt = outstanding_debt - ? WHERE id = ?");
        $stmt_c->bind_param('di', $dec, $custId);
        $stmt_c->execute();
      }
            $conn->commit();
            logActivity($conn,"Korosho debt payment: ".$debt['customer_name']." — ".number_format($amount)." TZS",'debt');
    ...
    if ($amount <= 0 || $cname === '') {
      ...
      try {
        if ($cid) {
          $stmt_cust = $conn->prepare("SELECT name FROM korosho_customers WHERE id = ?");
        ...
        $stmt = $conn->prepare("INSERT INTO korosho_debts (customer_id,customer_name,amount,balance,description,debt_date,due_date) VALUES (?,?,?,?,?,CURDATE(),?)");
        $stmt->bind_param('isddss',$cid,$cname,$amount,$amount,$desc,$due);
        $stmt->execute();
        if ($cid) {
          $stmt_up = $conn->prepare("UPDATE korosho_customers SET outstanding_debt = outstanding_debt + ? WHERE id = ?");
          $stmt_up->bind_param('di', $amount, $cid);
          $stmt_up->execute();
        }
        $conn->commit();
            logActivity($conn,"Korosho debt recorded: $cname — ".number_format($amount)." TZS",'debt');
    ...
    if ($debt_id > 0) {
      ...
      try {
        ...
        if ($debt['sale_id']) {
          throw new Exception('This debt is linked to a sale — void the sale in Reports instead.');
        }
        if ($debt['customer_id']) {
          $stmt_c = $conn->prepare("UPDATE korosho_customers SET outstanding_debt = outstanding_debt - ? WHERE id = ?");
        ...
        $stmt_dp = $conn->prepare("DELETE FROM korosho_debt_payments WHERE debt_id = ?");
        $stmt_dp->bind_param('i', $debt_id);
        $stmt_dp->execute();
        $stmt_d = $conn->prepare("DELETE FROM korosho_debts WHERE id = ?");
        ...
        logActivity($conn, "Korosho debt deleted: " . $debt['customer_name'] . " — " . number_format($debt['amount']) . " TZS" . $paid_note, 'debt');
    ...
    $_SESSION['korosho_debts_flash'] = $msg;
    header('Location: korosho_debts.php');
    exit;
}
$debts = $conn->query("SELECT * FROM korosho_debts ORDER BY FIELD(status,'outstanding','partial','cleared'), debt_date ASC");
$stats = $conn->query("SELECT COALESCE(SUM(balance),0) as total, COUNT(*) as count, SUM(status='partial') as partial, SUM(DATEDIFF(CURDATE(),due_date)>0 AND status!='cleared') as overdue FROM korosho_debts WHERE status != 'cleared'")->fetch_assoc();
$customers_list = $conn->query("SELECT id,name FROM korosho_customers WHERE status = 'active' ORDER BY name");
?>
...
<div class="card"><div class="card-header"><span class="card-title">Korosho Debts</span></div>
...
?>
<tr <?php if(!$cleared): ?>style="cursor:pointer<?=$overdue?';background:rgba(239,68,68,0.04)':''?>" onclick="koroshoPayDebt(<?=$d['id']?>,<?=$d['balance']?>,'<?=htmlspecialchars($d['customer_name'], ENT_QUOTES)?>','<?=$d['due_date']?:''?>')"<?php else: ?>style="opacity:0.65"<?php endif; ?>>
...
<?php if(!$cleared): ?><button class="btn btn-success btn-sm" onclick="event.stopPropagation();koroshoPayDebt(<?=$d['id']?>,<?=$d['balance']?>,'<?=htmlspecialchars($d['customer_name'], ENT_QUOTES)?>','<?=$d['due_date']?:''?>')">Pay</button><?php endif; ?>
<?php if(!$d['sale_id']):
...
<div class="form-row">
<div class="form-group"><label class="form-label">Customer</label><select name="customer_id" class="form-control" onchange="koroshoSetDebtName(this)"><option value="">-- Walk-in/Other --</option><?php $customers_list->data_seek(0); while($c=$customers_list->fetch_assoc()):?><option value="<?=$c['id']?>"><?=htmlspecialchars($c['name'])?></option><?php endwhile;?></select></div>
<div class="form-group"><label class="form-label">Customer Name *</label><input name="customer_name" id="korosho-debt-cname" class="form-control" placeholder="Full name" required/></div></div>
...
<!-- Pay Debt Modal -->
<div class="modal-overlay" id="pay-korosho-debt-modal" data-dismiss="true"><div class="modal"><div class="modal-header"><span class="modal-title">Record Payment</span><button class="modal-close" onclick="closeModal('pay-korosho-debt-modal')">&#x2715;</button></div>
<form method="POST"><input type="hidden" name="action" value="pay"><input type="hidden" name="debt_id" id="pay-korosho-debt-id"><?= csrfInput() ?>
<div class="modal-body">
<div style="margin-bottom:14px;padding:14px;background:var(--bg3);border-radius:10px"><div style="font-size:13px;color:var(--text2)">Customer: <strong id="pay-korosho-cname" style="color:var(--text)"></strong></div><div style="font-size:13px;color:var(--text2);margin-top:4px">Balance: <strong id="pay-korosho-balance" style="color:var(--danger)"></strong></div></div>
<div class="form-row"><div class="form-group"><label class="form-label">Amount Paying (TZS) *</label><input type="number" name="amount" id="pay-korosho-amount" class="form-control" required/></div><div class="form-group"><label class="form-label">Payment Method</label><select name="payment_method" class="form-control"><option value="cash">Cash</option><option value="mpesa">M-Pesa</option></select></div></div>
<div class="form-group"><label class="form-label">Due Date (for remaining balance)</label><input type="date" name="due_date" id="pay-korosho-due-date" class="form-control"/></div>
</div><div class="modal-footer"><button type="button" class="btn btn-outline" onclick="closeModal('pay-korosho-debt-modal')">Cancel</button><button type="submit" class="btn btn-success">Record Payment</button></div></form></div></div>

<?php
$extra_js = <<<'JS'
<script>
function koroshoPayDebt(id, balance, name, dueDate){
  document.getElementById('pay-korosho-debt-id').value = id;
  document.getElementById('pay-korosho-amount').value = balance;
  document.getElementById('pay-korosho-cname').textContent = name;
  document.getElementById('pay-korosho-balance').textContent = 'TZS '+balance.toLocaleString();
  document.getElementById('pay-korosho-due-date').value = dueDate || '';
  openModal('pay-korosho-debt-modal');
}
function koroshoSetDebtName(sel){
  const name = sel.selectedOptions[0].text;
  if(sel.value) document.getElementById('korosho-debt-cname').value = name;
}
</script>
JS;
