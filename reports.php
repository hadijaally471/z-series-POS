...
if ($period_type === 'daily') {
    for ($h = 0; $h < 24; $h++) {
        $stmt = $conn->prepare("SELECT COALESCE(SUM(total),0) as v FROM sales WHERE DATE(created_at)=? AND HOUR(created_at)=? AND status='completed'");
        $stmt->bind_param('si', $range_start, $h);
        $stmt->execute();
        $r = $stmt->get_result()->fetch_assoc()['v'];
        $daily[] = ['date' => str_pad((string)$h, 2, '0', STR_PAD_LEFT) . ':00', 'revenue' => $r];
    }
...
<tbody><?php $i=1; while($row=$city_sales_revenue->fetch_assoc()):?><tr><td class="text-muted"><?=$i++?></td><td class="td-bold"><?=htmlspecialchars($row['city'])?></td><td><?=number_format($row['cnt'])?></td><td class="text-success"><?=tzs($row['total'])?></td></tr><?php endwhile; if($city_sales_revenue->num_rows===0):?><tr><td colspan="4" style="text-align:center;padding:22px;color:var(--text3)">No city sales data for this period</td></tr><?php endif;?></tbody></table></div></div>
...
<div class="table-wrap reports-table-wrap"><table><thead><tr><th>#</th><th>City</th><th>Transactions</th><th>Revenue</th></tr></thead>
<tbody><?php $i=1; while($row=$city_sales_count->fetch_assoc()):?><tr><td class="text-muted"><?=$i++?></td><td class="td-bold"><?=htmlspecialchars($row['city'])?></td><td><?=number_format($row['cnt'])?></td><td class="text-success"><?=tzs($row['total'])?></td></tr><?php endwhile; if($city_sales_count->num_rows===0):?><tr><td colspan="4" style="text-align:center;padding:22px;color:var(--text3)">No city sales data for this period</td></tr><?php endif;?></tbody></table></div></div>
...
<tbody><?php $i=1; while($row=$city_product_leaders->fetch_assoc()):?><tr><td class="text-muted"><?=$i++?></td><td class="td-bold"><?=htmlspecialchars($row['city'])?></td><td><?=htmlspecialchars($row['product_name'])?></td><td><?=number_format($row['total_qty'])?></td><td class="text-success"><?=tzs($row['total_revenue'])?></td></tr><?php endwhile; if($city_product_leaders->num_rows===0):?><tr><td colspan="5" style="text-align:center;padding:22px;color:var(--text3)">No city/product sales data for this period</td></tr><?php endif;?></tbody></table></div></div>
<?php
$extra_js = <<<JS
<script>
